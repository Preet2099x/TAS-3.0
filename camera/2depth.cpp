...
// Cooldown after any serial command is sent (ms)
...
static bool serial_try_open(const char *port, int baud)
{
...
// Tries each candidate port in order, uses the first that opens.
static bool serial_autodetect(int baud)
{
    for (int i = 0; SERIAL_CANDIDATES[i] != nullptr; ++i)
    {
        const char *port = SERIAL_CANDIDATES[i];
        if (serial_try_open(port, baud))
        {
            std::cout << "Serial open: " << port
                      << " @ " << baud << " baud\n";
            return true;
        }
        std::cout << "Serial: " << port << " not available, trying next...\n";
    }
    std::cerr << "Serial: no port found. Is the Teensy connected?\n";
    return false;
}

// Sends "NNN\n" to Teensy
static void serial_send(int cmd)
{
    if (g_serial_fd < 0)
        return;
    ...
    if (write(g_serial_fd, buf, static_cast<size_t>(len)) < 0)
        std::cerr << "serial_send: write failed � " << strerror(errno) << '\n';
}
...
int main()
{
    try
    {
        if (!serial_autodetect(SERIAL_BAUD))
            return 1;
